...
@section('title', 'Historique Mouvements Stock - Bougies')
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="mb-8">
        ...
        <p class="text-gray-400 mt-2">Suivi des entrées et sorties de stock des bougies</p>
    </div>
    ...
    <div class="bg-gray-800/50 border border-gray-700/50 rounded-2xl p-6 mb-8">
        <form method="GET" action="{{ route('admin.mouvements.index') }}" class="grid grid-cols-1 md:grid-cols-6 gap-4">
            <!-- Type -->
            <div>
            ...
            <!-- Bougie -->
            <div>
                <label class="block text-sm font-medium text-gray-400 mb-2">Bougie</label>
                <select name="bougie_id" class="w-full bg-gray-900/50 border border-gray-700 rounded-xl px-4 py-2 text-gray-300 focus:border-purple-500 focus:ring-purple-500">
                    <option value="">Toutes</option>
                    @foreach($bougies as $bougie)
                        <option value="{{ $bougie->id }}" {{ request('bougie_id') == $bougie->id ? 'selected' : '' }}>
                            {{ $bougie->nom }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="flex items-end gap-2">
                ...
                <a href="{{ route('admin.mouvements.index') }}" class="px-4 py-2 bg-gray-700 text-gray-300 rounded-xl hover:bg-gray-600 transition">
                    Réinit
                </a>
            </div>
        </form>
            ...
            <table class="w-full">
                <thead class="bg-gray-900/50">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-left text-xs font-medium text-gray-400 uppercase">Bougie</th>
                ...
                <tbody class="divide-y divide-gray-700">
                    @forelse($mouvements as $mouvement)
                        <tr class="hover:bg-gray-700/30 transition">
                            ...
                            <td class="px-6 py-4">
                                @php
                                    $bougie = $bougies->firstWhere('id', $mouvement->produit_id);
                                @endphp
                                <span class="text-sm text-gray-300">{{ $bougie?->nom ?? 'Bougie #' . $mouvement->produit_id }}</span>
                                <div class="text-xs text-gray-500">{{ $bougie?->reference ?? 'N/A' }}</div>
                            </td>
                            <td class="px-6 py-4">
